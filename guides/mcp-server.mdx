...
With a **production key**, you can extend the flow: post a job → `invite_to_apply` → `list_applications` → `send_message` for follow-ups. See [Invite to apply](/guides/invite-to-apply), [Send a message](/guides/send-message), and [List applications](/guides/list-applications).
...
- Write tools (`post_job`, `invite_to_apply`, `send_message`, `delete_job`) instruct agents to confirm with you before calling.
