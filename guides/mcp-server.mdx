...
<Note>
  You need a Cutshort API key (`cs_live_…`) first. Approved employers can generate one under **Settings → Integrations → API Key** — start with **Eval access**. See [Getting API access](/introduction#getting-api-access).
</Note>
...
To attribute agent actions to a specific workspace member, add an `On-Behalf-Of: <cutshort-user-id>` header alongside `Authorization`:

```bash
npx mcp-remote https://mcp.cutshort.io/mcp \
  --header "Authorization: Bearer $CUTSHORT_API_KEY" \
  --header "On-Behalf-Of: $RECRUITER_USER_ID"
```
