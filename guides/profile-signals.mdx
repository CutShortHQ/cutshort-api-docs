...
`industryTags` lists domain experience (e-commerce, B2B SaaS, fintech, etc.) with `label`, `score`, and `reason`. Only tags with score ≥ 2 are included.

## Mapping to search filters

| Signal / level | Search param |
| --- | --- |
| `workQuality.level = high` | `workQual=high` |
| `careerGrowth.level = fast` | `carGrowth=fast` |
| `startupExperience.level = funded_startup` | `startupExp=funded_startup` |
| `entrepreneurship.level = cofounder` | `entrExp=cofounder` |

When requirements are preferences (not hard filters), run a broader search and sort by the relevant `profileSignals` field instead of over-narrowing filters.
